...
class Offre_image_part extends StorageManager {
	...
	public function getListe( $tab=array(), $debug=false ){
		$this->dbConnect();
		$requete = "SELECT * FROM `offre_image_part`" ;
	...
	public function ajouter( $value, $debug=false ) {
		...
		try {
			...
			$sql = "INSERT INTO `offre_image_part`
	...
	public function supprimer( $num_image=0, $debug=false ) {
		...
		if ( 1 == 1 ) {
			...
			$fichier_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . "/particuliers/photos/offre/grande" . $data[ 0 ][ "fichier" ];
			if ( file_exists( $fichier_a_supprimer ) ) {
				if ( $debug ) echo "On supprime " . $fichier_a_supprimer . "<br>\n";
				if ( !$debug ) unlink( $fichier_a_supprimer );
			}
			$fichier_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . "/particuliers/photos/offre/normale" . $data[ 0 ][ "fichier" ];
			if ( file_exists( $fichier_a_supprimer ) ) {
				if ( $debug ) echo "On supprime " . $fichier_a_supprimer . "<br>\n";
				if ( !$debug ) unlink( $fichier_a_supprimer );
			}
			$fichier_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . "/particuliers/photos/offre/vignette" . $data[ 0 ][ "fichier" ];
			if ( file_exists( $fichier_a_supprimer ) ) {
				if ( $debug ) echo "On supprime " . $fichier_a_supprimer . "<br>\n";
				if ( !$debug ) unlink( $fichier_a_supprimer );
			}
		...
		$this->begin();
		try {
			$sql = "DELETE FROM `offre_image_part` WHERE `num_image` = " . intval( $num_image );
			if ( $debug ) echo $sql . "<br>";
			else {
				$result = mysqli_query( $this->mysqli, $sql );
	...
	public function setChamp( $champ, $valeur=0, $num_image=0, $debug=false ) {
		...
		$this->begin();
		try {
			$requete = "UPDATE `offre_image_part` SET";
