...
class Offre_part extends StorageManager {
	...
	public function getListe( $tab=array(), $debug=false ){
		$this->dbConnect();
		$requete = "SELECT * FROM `offre_part`" ;
	...
	public function getListeAvecTypeBien( $tab=array(), $debug=false ){
		$this->dbConnect();
		$requete = "SELECT * FROM `offre_part` " ;
		$requete .= "INNER JOIN `offre_type_bien_part` ON offre_type_bien_part.num_offre = offre_part.num_offre" ;
		
		if ( $tab[ "where" ] == '' ) {
			$requete .= " WHERE offre_part.num_offre > 0 AND online='oui' ";
			...
			$order_by = ( $tab[ "order_by" ] != "" ) ? $tab[ "order_by" ] : "offre_part.num_offre";
			$sens = ( $tab[ "sens" ] != "" ) ? $tab[ "sens" ] : "ASC";
			$requete .= " ORDER BY " . $order_by . " " . $sens;
		}
		else $requete .= $tab[ "where" ];
	...
	public function ajouter( $value, $debug=false ) {
		...
		try {
			...
			$sql = "INSERT INTO `offre_part`
				(`titre`, `surface`, `description`, `prix`,  `bilan_energie`,`bilan_ges`,`nb_piece`, `a_la_une`, `online`)
				VALUES (
	...
	public function modifier( $value, $debug=false ) {
		...
		try {
			...
			$sql = "UPDATE `offre_part` SET
				...
				`bilan_ges` = '" . addslashes( $value[ "bilan_ges" ] ) . "',
				`a_la_une` = '" . $a_la_une ."'	 ,
				`online` = '" . $online ."'	 
	...
	public function supprimer( $num_offre, $debug=false ) {
		...
		try {
			...
			$sql = "DELETE FROM .`offre_part` WHERE `num_offre`=" . $num_offre . ";";
			if ( $debug ) echo $sql . "<br>";
			else {
				$result = mysqli_query( $this->mysqli, $sql );
